...
function shuffle(): void {
  for (let i: number = 0; colorCandidate.length > 0; i += i) {
    color = color.concat(colorCandidate.splice(Math.floor(Math.random() * colorCandidate.length)));
  }
}
    ...
    card.addEventListener('click', function (this: HTMLDivElement) {
      if (clickFlag && !completedCard.includes(this)) {
        ...
        if (clickCard.length === 2) {
          //null이 있다고 확신하면 as HTMLDivElement
